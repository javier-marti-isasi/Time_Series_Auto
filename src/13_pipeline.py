...
PIPELINE_NAME = "ClearML Simple Pipeline Demo 02"


# Este paso genera una lista de numeros aleatorios.
# En un pipeline real aqui podrias cargar datos, leer un parquet o descargar un Dataset de ClearML.
...
# Este paso transforma los datos.
# Aqui simulamos una limpieza o feature engineering muy simple multiplicando los valores.
...
# Este paso hace un "entrenamiento" simbolico.
# No entrena un modelo real: calcula estadisticas simples para que puedas entender el flujo del pipeline.
...
# Este paso evalua el resultado del paso anterior.
# En un caso real aqui calcularias metricas de validacion o test.
...
# Los parametros que aparecen aqui quedan registrados en ClearML y puedes cambiarlos desde la UI.
...
def simple_learning_pipeline(
    ...
    # Esta impresion aparece en los logs del pipeline principal.
    print("Pipeline finished successfully")
    print(f"Final evaluation: {evaluation}")


def main() -> None:
    # `run_locally()` hace que los pasos se ejecuten localmente, pero sigan apareciendo como pipeline en ClearML.
    # Es ideal para aprender antes de usar agentes remotos.
